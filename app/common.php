...
function check_user_action($object = "", $count_limit = 1, $ip_limit = false, $expire = 0)
{
    ...
    if ($find_log) {
        ...
        $updata['count'] = Db::raw('count+1');
        $updata['last_time'] = $time;
        $updata['ip'] = $ip;
        Db::name("action_log")->where($where)->update($updata);
    } else {
        Db::name("action_log")->insert(array("uid" => $userid, "action" => $action, "object" => $object, "count" => Db::raw('count+1'), "last_time" => $time, "ip" => $ip));
    }
    return true;
}
